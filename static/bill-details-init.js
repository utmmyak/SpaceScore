$( document ).ready(function() {
	...
	setCookie('csrftoken', $('input[name="csrfmiddlewaretoken"]').val(), 1);
    ...
    var chartElement = $("#bill-weight-chart");
    var chartData = {
	...
	var alreadyVoted = function(){
		//$('#content > div:nth-child(1) > div.col-xs-7 > button').prop("disabled", true);
		$("#vote-alert").css("display", "block");
		$('#vote-msg').text("You have already voted on this bill.");
		$("#alert-glyph").removeClass("glyphicon-ok-sign");
		$("#usr-vote-form > table > tbody > tr > td > input").prop("disabled", "true");
		$("#alert-glyph").addClass("glyphicon-exclamation-sign");
	...
	ec.get(document.URL + "vote/",
		function(res) {
			console.log("got to cookie");
			console.log("cookie response is " + res);
			if (res == "true") {
				alreadyVoted();
			}
		}
    ...
    $(".tooltipster-icon.basescore-tooltip.popover").popover({
        html: true,
        content: "<span>The Base Score reflects our editorial opinion of how closely this bill aligns with the needs of space in general as outlined <a href=\"/methodology.html#base\">here</a>.</span>",
        trigger: 'manual',
        placement: 'bottom',
